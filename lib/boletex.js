boletex = {
	formata : function(linha) {
		return linha.replace(/[^0-9]/g,'');
	},
	calculaBarra : function(linha)
	{
		barra  = this.formata(linha)
	...
	calculaLinha : function (barra)
	{
		linha = this.formata(barra);
	...
	reboleta : function (linha, newData)
	{
		...
		var barra = this.changeFator(this.calculaBarra(this.formata(linha)), this.dataToFator(newData));

		//Calcula o digito verificador do novo código de barras e altera na barra e retorna
		return this.calculaLinha(this.changeDv(barra, this.modulo11Banco(barra)));
	}
}